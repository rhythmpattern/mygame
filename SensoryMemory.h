...
private:
  ...
  void       MakeNewRecordIfNotAlreadyPresent(Character* pBot);

public:

  SensoryMemory(Character* owner, double MemorySpan);
  ...
  void     RemoveBotFromMemory(Character* pBot);
  ...
  double    GetTimeOpponentHasBeenVisible(Character* pOpponent)const;
  double    GetTimeSinceLastSensed(Character* pOpponent)const;
  double    GetTimeOpponentHasBeenOutOfView(Character* pOpponent)const;

  //this method returns a list of all the opponents that have had their
  //records updated within the last m_dMemorySpan seconds.
  std::vector<Character*> GetListOfRecentlySensedOpponents()const;
