...
{
  Vector2D ToTarget = target - m_pCharacter->Pos();
  ...
  if (dist > 0)
  {
    ...
    speed = MinOf(speed, m_pCharacter->MaxSpeed());
...
{ 
  ...
  Vector2D Target = PointToWorldSpace(target,
                                       m_pCharacter->Heading(),
                                       m_pCharacter->Side(), 
                                       m_pCharacter->Pos());

  //and steer towards it
  return Target - m_pCharacter->Pos(); 
}
...
//------------------------------------------------------------------------
Vector2D Steering::Separation(const std::vector<Character*>& neighbors)
{  
  //iterate through all the neighbors and calculate the vector from the
  Vector2D SteeringForce;

  std::vector<Character*>::const_iterator it = neighbors.begin();
  for (it; it != neighbors.end(); ++it)
  {
    //make sure this agent isn't included in the calculations and that
    //the agent being examined is close enough. ***also make sure it doesn't
    //include the evade target ***
    if((*it != m_pCharacter) && (*it)->IsTagged() &&
      (*it != m_pTargetAgent1))
    {
      Vector2D ToAgent = m_pCharacter->Pos() - (*it)->Pos();
